...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; } header { background:#135c3c; color:#fff; padding:22px max(20px, calc((100% - 1180px)/2)); }
    ...
    label { display:grid; gap:6px; font-size:13px; font-weight:700; } input, select { min-height:38px; width:100%; border:1px solid #adc1b3; border-radius:4px; padding:8px; font:inherit; }
...
<body>
  ...
  <script>
    ...
    async function quickPrice(article){const item=registry.find(value=>value.article===article);const price=document.querySelector(`[data-price="${CSS.escape(article)}"]`).value;try{setMessage('Сохранение цены...');const data=await jsonApi('/api/sets/update',{...credentials(),...item,article,discounted_price:price});if(!data.success)throw new Error(data.message);setMessage('Цена обновлена.','ok');await loadState()}catch(error){setMessage(error.message,'error')}}
    async function saveEditor(){try{setMessage('Сохранение набора...');const data=await jsonApi('/api/sets/update',{...credentials(),article:document.getElementById('edit-article').value,title:document.getElementById('edit-title').value,display_articles:document.getElementById('edit-products').value,discounted_price:document.getElementById('edit-price').value,currency:document.getElementById('edit-currency').value,sort_order:document.getElementById('edit-sort').value,discount_percent:document.getElementById('edit-discount').value,enabled:document.getElementById('edit-enabled').value});if(!data.success)throw new Error(data.message);setMessage('Набор обновлен.','ok');document.getElementById('editor').classList.remove('visible');await loadState()}catch(error){setMessage(error.message,'error')}}
    async function deleteSelected(){const articles=[...document.querySelectorAll('[data-select]:checked')].map(node=>node.dataset.select);if(!articles.length){setMessage('Выберите наборы для удаления.','error');return}if(!confirm(`Удалить наборов: ${articles.length}? Это действие будет отправлено в Хорошоп.`))return;try{setMessage('Удаление наборов...');const data=await jsonApi('/api/sets/delete',{...credentials(),articles});const failed=data.items.filter(item=>!item.success);setMessage(failed.length?`Удаление завершено с ошибками: ${failed.map(item=>item.article).join(', ')}`:'Выбранные наборы удалены. ',failed.length?'error':'ok');await loadState()}catch(error){setMessage(error.message,'error')}}
    async function fileAction(endpoint, importing){const file=document.getElementById('file').files[0];if(!file){setMessage('Выберите Excel-файл.','error');return}const body=new FormData();body.append('file',file);for(const [key,value] of Object.entries(credentials()))body.append(key,value);try{setMessage(importing?'Выполнение операций...':'Проверка файла...');const response=await fetch(endpoint,{method:'POST',body});const data=await readResponse(response);if(!response.ok)throw new Error(data.detail||'Ошибка сервера.');renderResult(data.items||[]);setMessage(importing?`Готово: успешно ${data.imported}, ошибок ${data.errors}.`:`Готово: без ошибок ${data.ready}, ошибок ${data.errors}.`,data.errors?'error':'ok');if(importing)await loadState()}catch(error){setMessage(error.message,'error')}}
